...
export default async function handler(req, res) {
  ...
  try {
    ...
    // GET 요청: 드라마 정보 조회
    if (req.method === 'GET') {
      const drama = await dramaCollection.findOne({ _id: objectId });
      
      if (!drama) {
        return res.status(404).json({ success: false, message: '해당 드라마를 찾을 수 없습니다.' });
      }
    ...
    if (req.method === 'PUT') {
      ...
      const result = await dramaCollection.updateOne(
        { _id: objectId },
        { $set: updateData }
      );
      ...
      return res.status(200).json({ 
        success: true, 
        message: '드라마 정보가 성공적으로 업데이트되었습니다.',
    ...
    if (req.method === 'DELETE') {
      const result = await dramaCollection.deleteOne({ _id: objectId });
    ...
    console.error('드라마 API 처리 오류:', error);
