...
제목: [아티스트], 마이원픽 '[시즌]' 원픽차트 [부문] 1위, [SEO 타이틀]

1문단 (수상 현황):
"[아티스트]가 글로벌 팬덤 플랫폼 마이원픽(my1pick)이 발표한 '[시즌] 원픽차트(1PICK CHART)' [부문]에서 1위를 차지했다.
[득표율/표차 수치]의 높은 득표율로 정상에 오르며 팬들의 열렬한 지지를 받았다."

2문단 (원픽차트 소개 - 고정):
"마이원픽 원픽차트(1PICK CHART)는 글로벌 팬들이 각 부문에서 단 한 명의 아티스트를 선택하는 팬 투표 차트로, KPOP(Solo), KPOP(Group), TROT, CELEB, GLOBAL, TREND 등 다양한 부문으로 운영된다.
각 부문 1위 아티스트에게는 온라인 어워즈 시상과 함께 수상 콘텐츠가 마이원픽 공식 유튜브를 통해 공개되며, 실물 트로피도 전달된다."
...
// 고정 문단 (팩트체크용)
const FIXED_PARAGRAPH_2 = "마이원픽 원픽차트(1PICK CHART)는 글로벌 팬들이 각 부문에서 단 한 명의 아티스트를 선택하는 팬 투표 차트로, KPOP(Solo), KPOP(Group), TROT, CELEB, GLOBAL, TREND 등 다양한 부문으로 운영된다. 각 부문 1위 아티스트에게는 온라인 어워즈 시상과 함께 수상 콘텐츠가 마이원픽 공식 유튜브를 통해 공개되며, 실물 트로피도 전달된다.";
...
async function factCheckWithClaude(article, voteData) {
  ...
  // 상위 후보자 순위 정보
  const rankingInfo = voteData.topRankings?.map((r) =>
    `${r.rank}위: ${r.starName || r.displayName} - ${r.useQty?.toLocaleString()}표 (${r.votePercent}%)`
  ).join('\n') || '';

  // 2위와의 득표수 차이
  const secondPlace = voteData.topRankings?.[1];
  const voteDifference = secondPlace ? (voteData.winnerVotes - secondPlace.useQty) : 0;

  // 전체 득표수 합계
  const totalAllVotes = voteData.totalVotes || 0;
...
- 1위 아티스트: ${voteData.artistName}
- 1위 득표수: ${voteData.winnerVotes?.toLocaleString()}표
- 1위 득표율: ${voteData.votePercentage}%
- 전체 득표수 합계: ${totalAllVotes.toLocaleString()}표
- 투표 기간: ${voteData.startDate} ~ ${voteData.endDate}
${secondPlace ? `- 2위: ${secondPlace.starName || secondPlace.displayName} - ${secondPlace.useQty?.toLocaleString()}표` : ''}
...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (!chart || !chart.rankings || chart.rankings.length === 0) {
      return res.status(400).json({ success: false, message: '차트 및 순위 데이터가 필요합니다.' });
    }

    const winner = chart.rankings[0];
    const totalVotes = chart.totalVotes;
    const votePercentage = winner.votePercent;

    // 시즌 파싱 (e.g., "26.2" → "2026년 2월")
    const seasonParts = chart.season.match(/^(\d+)\.(\d+)$/);
    const seasonDisplay = seasonParts
      ? `20${seasonParts[1]}년 ${seasonParts[2]}월`
      : chart.season;

    const category = chart.chartType;
    ...
    // 2위 정보
    const secondPlace = chart.rankings[1];
    const voteDifference = secondPlace ? (winner.useQty - secondPlace.useQty) : 0;

    // Claude에게 기사 생성 요청
    const articlePrompt = `당신은 매우 뛰어난 추론 능력을 갖춘 K-pop 전문 기자입니다.
마이원픽 원픽차트(1PICK CHART) 수상 기사를 작성하기 전에, 먼저 체계적으로 계획을 세우고 기사를 작성해주세요.
...
- 시즌: ${seasonDisplay}
- 부문: ${category}
- 1위 아티스트: ${winner.starName || winner.displayName}
- 득표수: ${winner.useQty?.toLocaleString()}표
- 득표율: ${votePercentage}%
- 전체 투표수: ${totalVotes?.toLocaleString()}표
- 투표 기간: ${chart.startAt} ~ ${chart.endAt}
${secondPlace ? `- 2위: ${secondPlace.starName || secondPlace.displayName} - ${secondPlace.useQty?.toLocaleString()}표 (${secondPlace.votePercent}%)` : ''}
${secondPlace ? `- 1위와 2위 표차: ${voteDifference.toLocaleString()}표` : ''}

## 순위 (상위 5명):
${chart.rankings.slice(0, 5).map((r) => `${r.rank}위: ${r.starName || r.displayName} - ${r.useQty?.toLocaleString()}표 (${r.votePercent}%)`).join('\n')}
...
- **고정 문구 (변경 불가)**: 2문단(원픽차트 소개), 5문단(마이원픽 소개) - 템플릿 그대로 사용
- **작성 필요 정보**: 3, 4문단에 들어갈 아티스트의 최근 활동 정보

### 1.2 아티스트 정보 분석
"${winner.starName || winner.displayName}"에 대해 당신이 알고 있는 **검증된 최신 정보**만 사용하세요:
...
2. **1문단**: 제공된 투표 데이터(득표수, 득표율)를 정확히 반영. "원픽차트(1PICK CHART)"로 명시
3. **2문단**: 아래 고정 문구 그대로 사용 (수정 금지):
"마이원픽 원픽차트(1PICK CHART)는 글로벌 팬들이 각 부문에서 단 한 명의 아티스트를 선택하는 팬 투표 차트로, KPOP(Solo), KPOP(Group), TROT, CELEB, GLOBAL, TREND 등 다양한 부문으로 운영된다. 각 부문 1위 아티스트에게는 온라인 어워즈 시상과 함께 수상 콘텐츠가 마이원픽 공식 유튜브를 통해 공개되며, 실물 트로피도 전달된다."
...
기사를 작성해주세요:`;
    ...
    const factCheckResult = await factCheckWithClaude(generatedArticle, {
      campaignTitle: chart.title,
      season: seasonDisplay,
      category,
      artistName: winner.starName || winner.displayName,
      winnerVotes: winner.useQty,
      votePercentage,
      totalVotes,
      startDate: chart.startAt,
      endDate: chart.endAt,
      topRankings: chart.rankings.slice(0, 5)
    });
    ...
    const extractedTitle = titleMatch
      ...
      : `${winner.starName || winner.displayName}, 마이원픽 '${seasonDisplay}' 원픽차트 ${category} 1위`;
    ...
    return res.status(200).json({
      ...
      data: {
        ...
        factCheckResult,
        voteData: {
          campaignIdx: chart.campaignIdx,
          campaignTitle: chart.title,
          season: seasonDisplay,
          category,
          artistName: winner.starName || winner.displayName,
          groupName: null,
          totalVotes: winner.useQty,
          votePercentage,
          startDate: chart.startAt,
          endDate: chart.endAt,
          topRankings: chart.rankings.slice(0, 5)
    ...
    console.error('Onepick article generation error:', error);
