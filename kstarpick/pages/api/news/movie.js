...
if (!MONGODB_URI) {
  console.error('[Movie API] MONGODB_URI environment variable is not defined');
 ...
 * /api/news/movie:
 *   get:
 *     description: 영화 관련된 뉴스 글들을 가져옵니다
 *     parameters:
 *       - name: page
 *         in: query
 ...
 *     responses:
 *       200:
 *         description: 영화 뉴스 목록과 페이지네이션 정보
 */
export default async function handler(req, res) {
  ...
  try {
    console.log('[Movie API] === 네이티브 MongoDB 연결 시작 ===');
    ...
    console.log('[Movie API] MongoDB 연결 성공');
    ...
    // 영화 카테고리 뉴스만 조회 (대소문자 무관) + 오류 뉴스 필터링
    const query = {
      $and: [
        { category: { $regex: '^movie$', $options: 'i' } },
        {
          $or: [
    ...
    // 디버깅 로그 추가
    console.log(`[API] 영화 뉴스 요청: 페이지 ${pageNum}, 개수 ${limitNum}, 건수 ${news.length}, 총 ${total}건`);
    ...
    console.error('[Movie API] 영화 뉴스 조회 오류:', error);
    ...
    if (client) {
      await client.close();
      console.log('[Movie API] MongoDB 연결 종료');
    }
  }
} 