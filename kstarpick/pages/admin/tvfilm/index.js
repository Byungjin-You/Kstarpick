  ...
  const handleMultiDeleteConfirm = async () => {
    ...
    try {
      ...
      if (!hasError) {
        ...
        setSelectedItems([]);
        showToast(`${selectedItems.length}개의 영화가 성공적으로 삭제되었습니다.`, 'success');
      } else {
        setError('일부 항목 삭제 중 오류가 발생했습니다.');
      }
    } catch (error) {
      console.error('Error deleting multiple movies:', error);
      setError('영화 삭제 중 오류가 발생했습니다.');
    } finally {
      setIsDeleting(false);
    }
  };
  ...
  return (
    <AdminLayout>
      <div className="mb-6 flex justify-between items-center">
        <div>
          <h1 className="text-2xl font-bold text-gray-800">영화 관리</h1>
        </div>
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-lg p-8 max-w-md w-full">
            ...
            <p className="text-sm text-gray-500 mb-4">
              선택한 {selectedItems.length}개의 영화를 삭제하시겠습니까? 이 작업은 되돌릴 수 없습니다.
            </p>
