 ...
 * 
 * 사용 방법:
 * 1. 드라마 제목을 인수로 전달: node scripts/addDrama.mjs "드라마 제목"
 * 2. 또는 여러 드라마를 함께 전달: node scripts/addDrama.mjs "드라마1" "드라마2"
 */
...
async function saveDramaToDB(db, dramaData) {
  try {
...
async function main() {
  ...
  try {
    ...
    if (dramaTitles.length === 0) {
      ...
      console.log('사용법: node scripts/addDrama.mjs "드라마 제목"');
      process.exit(1);
    }
    ...
    for (const title of dramaTitles) {
      ...
      if (dramaInfo) {
        await saveDramaToDB(db, dramaInfo);
      }
    }
